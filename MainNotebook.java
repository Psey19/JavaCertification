...
//4) У меня также была идея реализации программы работающей по следующему сценарию:
//   Внутри метода по фильтрации повторно вызвать этот метод (осуществить рекурсию), имитируя работу интенет-магазина в реальном времени,
//   где после выбора интересующей модели, пользователь выбирает покупку, после чего данный экземпляр удаляется из множества
//   notebooks, сокращая ассортимент, цикл повторяется снова, как-будто уже для нового пользователя, а когда, к примеру, экземпляры
//   с параметром ram: 32 заканчиваются, то из критерия выбора ОЗУ вариант 32ГБ также исчезает (для реализации полученных навыков по курсу достаточно)
